...
class ManipulationSystem {
  ...
  _dragControlNode(event) {
    ...
    if (this.temporaryIds.nodes[0] !== undefined) {
      let targetNode = this.body.nodes[this.temporaryIds.nodes[0]]; // there is only one templates node in the add edge mode.
  ...
  /**
   * Connect the new edge to the target if one exists, otherwise remove templates line
   * @param {Event}  event   The event
   * @private
   */
